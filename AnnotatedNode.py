...
def read_phyloXML(file, branch_lengths=True, keep_comments=False, ind="  ", indstart="\n"):
	...
	if l[start].startswith('<phyloxml'):
		...
		raise SyntaxError, "Invalid phyloXML syntax in file %s at line %d."%(file, start+1)
	if l[end]=='</phyloxml>':
		end -= 1
	else:
		raise SyntaxError, "Invalid phyloXML syntax in file %s at line %d."%(file, end+1)
	...
	for k in range(ntree):
		...
		if l[treestart].startswith('<phylogeny'):
			...
			raise SyntaxError, "Invalid phyloXML syntax in file %s at line %d."%(file, treestart+1)	
...
def read_obo(nfobo):
	"""reads in OBO XML file and parse OBO structure to make a tree of term hierarchy (developped for reading Gene Ontology [GO] terms hierarchy)"""
