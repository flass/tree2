	...
	def __copy__(self):
		...
		for attr in self.__dict__:
			if not attr in ['_Node__children', '_Node__father']:
				...
				newinstance.__dict__[attr] = copy.copy(self.__dict__[attr])
...
############## methods for editing object atributes:
	...
	def edit_label(self, label, mode="write"):
		...
		if mode in ["w", "write"] or self.__lab=="":
	...
	def complete_internal_labels(self, prefix='N', labels=None, force=False, excludeLeaves=False):
...
### External program call functions
	...
	def seaview(self, seaview_exec_path='seaview', tmpfiledir='%s/tmp'%homedir, filename='tmptree2seaview.phb', bg=True, **kw): #, ignoreBS=False, branch_lengths=True, unrooted=False, comment='comment'):
		ntmpfile = "%s/%s"%(tmpfiledir, filename)
...
########### distance measure methods:
	...
	def lineage_distance(self, n):
		"""Similar to Node.distance(), except that it substracts half of the nodes' branch lengths to give an approximate distance of the lineage"""
		d = self.distance(n)
		if d:
			for node in [self, n]:
				if node.lg():
...
############################# DEV
	...
	def unlink_child(self, child):
		self.rm_child(child)
		child.change_father(None)
	
	def change_father(self, newfat, newlen=None, newboot=None, silent=True):
		self.__father = newfat
		self.__l = newlen
		self.__boot = newboot
		if not silent: print "%s <- %s %s:%s"%(newfat.label(), self.label(), str(self.lg()), str(self.bs()))
